...
def print_chords_and_times(chords, startTimes, endTimes, frameIndex, times):
	...
	for i in range (frameIndex):
		print('frame : %6d | chord : %6s | startTime : %8.5f | endTime : %7.5f | length : %8.5f' % (i+1,chords[i],startTimes[i]+first_onset_time,endTimes[i]+first_onset_time, endTimes[i]-startTimes[i]))
